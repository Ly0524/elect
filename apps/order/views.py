    ...
    def post(self, request):
        ...
        try:
            ...
            for sku_id in sku_ids:
                ...
                try:
                    sku = Goods.objects.select_for_update().get(id=sku_id)
        ...
        try:
            ...
            for sku_id in sku_ids:
                for i in range(3):  # 尝试3次
                    ...
                    # 返回受影响的行数res, 0为失败
                    res = Goods.objects.filter(id=sku_id, stock=orgin_stock).update(stock=new_stock,
                                                                                       sales=new_sales)  # 乐观锁
                    if res == 0:  # 返回0表示更新失败
                        if i == 2:  # 尝试到第三次
                            transaction.savepoint_rollback(save_id)
